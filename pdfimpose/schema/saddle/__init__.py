...
class SaddleImpositor(perfect.PerfectImpositor):
    ...
    def matrixes(self, pages: int):
        ...
        matrixes = list(self.base_matrix())
        for i in range(pages // (2 * pages_per_signature)):
            yield from self.insert_sheets(
                (matrix.copy() for matrix in matrixes), i, pages
            )

    def insert_sheets(self, matrixes, sheets, pages):
        """TODO
        ...
        for matrix in matrixes:
            for x, y in matrix.coordinates():
                ...
                # Change page numbers
                signature_size = matrix.width * matrix.height
                if matrix[x, y].number < signature_size:
                    matrix[x, y].number += signature_size * sheets
                else:
                    matrix[x, y].number = (
                        pages - (sheets + 2) * signature_size + matrix[x, y].number
                    )
